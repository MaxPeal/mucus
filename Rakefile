...
BOX_BASENAME_IA64 = "#{BOX_BASENAME}-ia64"
...
BOX_IA64 = "#{BOX_BASENAME_IA64}.box"
...
SHORT_DESCRIPTION_IA64 = "#{SHORT_DESCRIPTION} ia64"
...
task :box_ia64 => [
    "ia64#{File::SEPARATOR}Vagrantfile",
    "ia64#{File::SEPARATOR}bootstrap.sh",
    "ia64#{File::SEPARATOR}export.Vagrantfile",
    :clean_box_ia64
] do
    sh 'vagrant up',
        :chdir => "ia64"
    sh "vagrant package --output #{BOX_IA64} --vagrantfile export.Vagrantfile",
        :chdir => "ia64"
end
...
task :import_ia64 => [] do
    sh "vagrant box add --force --name #{BOX_NAMESPACE}/#{BOX_BASENAME_IA64} #{BOX_IA64}",
        :chdir => "ia64"
end
...
task :test_ia64 => [
    "ia64#{File::SEPARATOR}test#{File::SEPARATOR}Vagrantfile",
    "ia64#{File::SEPARATOR}test#{File::SEPARATOR}hello.cpp"
] do
    sh 'vagrant up',
        :chdir => "ia64#{File::SEPARATOR}test"
    sh 'vagrant ssh -c "cd /vagrant && ia64-linux-gnu-g++ -o hello hello.cpp && qemu-ia64-static hello"',
        :chdir => "ia64#{File::SEPARATOR}test"
end
...
task :publish_ia64 => [] do
    sh "vagrant cloud publish #{BOX_NAMESPACE}/#{BOX_BASENAME_IA64} --force --release --short-description \"#{SHORT_DESCRIPTION_IA64}\" --version-description \"#{VERSION_DESCRIPTION}\" #{VERSION} #{PROVIDER} #{BOX_IA64}",
        :chdir => "ia64"
end
...
task :clean_box_ia64 => [] do
    Dir.glob("ia64#{File::SEPARATOR}*.box").each { |path| File.delete path }
end
...
task :clean_ia64 => [:clean_box_ia64] do
    begin
        sh 'vagrant destroy -f',
            :chdir => 'ia64'
    rescue
    ...
    begin
        Dir.glob("ia64#{File::SEPARATOR}**#{File::SEPARATOR}.vagrant").each { |path| FileUtils.rm_r path }
    rescue
    end

    begin
        FileUtils.rm_r "ia64#{File::SEPARATOR}_tmp_package"
    rescue
    end
end
